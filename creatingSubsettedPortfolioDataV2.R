...
getColumnsBasedOnSimilarGVKEY = function(rawDataColumnNames, GVKEYsToFind){
  ...
  patternsToFind = paste(paste(c(unique(c("date",GVKEYsToFind)))),collapse = "|",sep="")
  result = which(grepl(patternsToFind,rawDataColumnNames))
  return(result)
}
...
identifyIndicesThatMatchAPatternFromAVector = function(vectorToBeChecked,vectorToBeMatched){
  lenVectorToBeMatched = length(vectorToBeMatched)
  if(lenVectorToBeMatched<1000){return()}
}
...
onlyS_And_P = runTheProgram("filteredData","S&P500",theBounds)
