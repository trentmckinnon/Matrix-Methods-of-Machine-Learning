# hasLoadedTheData = "theData" %in% ls(all=T)
# if(hasLoadedTheData){
#   rm(list = ls(all=T)[-which(ls(all=T)=="hasLoadedTheData"|ls(all=T)=="theData")])
# }else{rm(list = ls(all=T))}
...
organizeData = function(unorganizedData){
  ...
  #   print(i)
  #   result[[counter]] = getOrganizedStock(unorganizedData[which(unorganizedData$gvkey == i),],uniqueDates)
  # }
  print(2)
  stopImplicitCluster()
